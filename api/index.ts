...
// Note: data.json will NOT persist on Vercel/Netlify. 
// It will reset every time the serverless function cold starts.
// You should use a real database like MongoDB or Supabase.
...
let data: any = {
  ...
  tasks: [
    ...
    { id: '19', title: 'Science Trivia', description: 'Answer a question about scientific question.', reward: 3, type: 'science' },
...
// Load existing data if any (Sync because it's startup)
...
const saveData = () => {
  // On serverless, this might fail or not persist
  try {
...
app.post('/api/user/daily-bonus', (req, res) => {
  ...
  if (user.lastBonusDate === today) return res.status(400).json({ error: 'Bonus already claimed today' });
  const bonusAmount = 5;
...
app.post('/api/withdraw', (req, res) => {
  ...
  if (!data.users[userId] || data.users[userId].balance < amount) return res.status(400).json({ error: 'Insufficient balance' });
  if (amount < 20) return res.status(400).json({ error: 'Minimum withdrawal is 20 BDT' });
  const withdrawal = {
...
app.get('/api/admin/stats', (req, res) => {
  ...
  const totalEarnings = Object.values(data.users).reduce((acc: number, u: any) => acc + (u.completedTasks?.length || 0) * 1.25, 0);
  res.json({ 
    totalUsers, 
    totalEarnings, 
